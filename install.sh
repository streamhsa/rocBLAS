...
pushd .
  ...
  if [[ "${build_clients}" == true ]]; then
    CXX=${compiler} ${cmake_executable} ${cmake_common_options} ${cmake_client_options} -DCPACK_SET_DESTDIR=OFF -DCMAKE_INSTALL_PREFIX=/opt/rocm-2.8.1 -DCPACK_PACKAGING_INSTALL_PREFIX=/opt/rocm-2.8.1 ../..
  else
    CXX=${compiler} ${cmake_executable} ${cmake_common_options} -DCPACK_SET_DESTDIR=OFF -DCMAKE_INSTALL_PREFIX=/opt/rocm-2.8.1 -DCPACK_PACKAGING_INSTALL_PREFIX=/opt/rocm-2.8.1 ../..
  fi
  check_exit_code
